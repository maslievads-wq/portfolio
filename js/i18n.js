/* ============================================================
   ...
   cases:  full localized case content x6 (numbers/tools come from CASE_META)
   ============================================================ */
...
const I18N = {
  ...
  en: {
    ui: {
      ...
      'case.viewStudy': 'View case study →',
      'case.challenge': 'Challenge',
      'case.actions': 'Actions Taken',
  ...
  de: {
    ui: {
      ...
      'case.viewStudy': 'Fallstudie ansehen →',
      'case.challenge': 'Herausforderung',
      'case.actions': 'Maßnahmen',
      'case.tools': 'Eingesetzte Tools',
  ...
  ru: {
    ui: {
      ...
      'hero.subtitle': 'Я строю масштабируемые маркетинговые системы, объединяю команды, организую аналитику и превращаю маркетинг в предсказуемый двигатель роста.',
    ...
    cases: [
      { tag: 'Рост маркетингового агентства', title: 'Масштабирование digital-агентства', desc: 'Перестроил системы привлечения и реализации агентства для предсказуемого прибыльного роста.', industry: 'Digital-услуги', challenge: 'Агентство полагалось на рекомендации и нерегулярный аутрич. Поток лидов был непредсказуем, маржинальность неясна, а системы отчётности для решений не было.', actions: ['Построил структурированную систему outbound + inbound привлечения', 'Внедрил CRM с отслеживанием воронки по этапам', 'Стандартизировал офферы, цены и процессы реализации', 'Ввёл еженедельные дашборды эффективности для руководства'], results: 'За два квартала агентство перешло от непредсказуемого дохода с рекомендаций к задокументированной воспроизводимой воронке — утроив число квалифицированных лидов и снизив стоимость привлечения на треть.', metricLabels: ['Рост выручки', 'Рост лидов', 'Снижение CAC'] },
      { tag: 'Recruitment-маркетинг', title: 'Система рекрутингового маркетинга', desc: 'Разработал бренд работодателя и воронку привлечения кандидатов для компании с быстрым наймом.', industry: 'HR / подбор персонала', challenge: 'Найм зависел от дорогих job-площадок с низким качеством откликов и без измеримой воронки.', actions: ['Построил лендинг-воронку для кандидатов с чёткими сообщениями', 'Запустил таргетированные платные кампании в соцсетях', 'Автоматизировал скрининг и цепочки касаний', 'Создал дашборд аналитики рекрутинга'], results: 'Стоимость квалифицированного найма снизилась на 42 %, а качество и объём откликов заметно выросли — команда получила предсказуемую воронку найма.', metricLabels: ['Квалифицированные отклики', 'Стоимость найма ↓', 'Конверсия'] },
      { tag: 'Образовательные проекты', title: 'Запуск EdTech и рост записей', desc: 'Провёл онлайн-образовательный продукт от идеи до стабильных ежемесячных записей.', industry: 'Образование', challenge: 'У нового курса не было ни аудитории, ни воронки, ни канала привлечения — только сильный контент.', actions: ['Определил позиционирование и оффер на основе ценности', 'Построил воронку «вебинар → продажа»', 'Запустил performance-кампании с точным трекингом', 'Внедрил email-прогрев и цепочки реактивации'], results: 'Продукт вышел на стабильные масштабируемые ежемесячные записи с положительной окупаемостью рекламы и воспроизводимым плейбуком запуска.', metricLabels: ['Рост записей', 'ROAS', 'Конверсия воронки'] },
      { tag: 'Performance-маркетинг', title: 'Масштабирование performance в e-commerce', desc: 'Перестроил платное привлечение для роста выручки с сохранением целей по прибыльности.', industry: 'E-commerce', challenge: 'Рекламные расходы росли, но ROAS падал, а у команды не было видимости реальной прибыльности.', actions: ['Перестроил структуру кампаний вокруг маржи продуктов', 'Внедрил сквозной трекинг и атрибуцию', 'Ввёл систему тестирования креативов', 'Оптимизировал лендинги и оформление заказа'], results: 'Эффективность прибыльных рекламных расходов выросла более чем вдвое, что позволило уверенно масштабироваться на основе понятной отчётности по марже.', metricLabels: ['Рост ROAS', 'Рост выручки', 'Рост конверсии'] },
      { tag: 'Системы лидогенерации', title: 'Движок B2B-лидогенерации', desc: 'Построил предсказуемую многоканальную систему B2B-лидов для сервисного бизнеса.', industry: 'B2B-услуги', challenge: 'У продаж не было стабильного входящего потока, и большую часть времени они тратили на ручной поиск.', actions: ['Спроектировал систему inbound-контента и лид-магнитов', 'Построил outbound-цепочки с обогащённым таргетингом', 'Связал формы, CRM и автоматизацию маршрутизации', 'Создал модель скоринга лидов на основе SLA'], results: 'Продажи перешли от ручного поиска к работе со стабильной квалифицированной воронкой — выросли и объём лидов, и конверсия в сделки.', metricLabels: ['Лидов в месяц', 'Экономия времени продаж', 'Лид → сделка ↑'] },
      { tag: 'Проекты AI-автоматизации', title: 'Маркетинг-операции на базе ИИ', desc: 'Внедрил ИИ и автоматизацию в контент, отчётность и обработку лидов.', industry: 'Технологии', challenge: 'Рутинные маркетинговые задачи съедали ресурс команды и замедляли отчётность и скорость ответа.', actions: ['Автоматизировал сбор отчётности по каналам', 'Внедрил ИИ-воркфлоу для контента и креативов', 'Построил ИИ-квалификацию и маршрутизацию лидов', 'Создал внутренние плейбуки автоматизации'], results: 'Автоматизация высвободила значительный ресурс команды и ускорила принятие решений — рутинные операции стали масштабируемой системой с низкими издержками.', metricLabels: ['Ручная работа ↓', 'Скорость отчётности', 'Быстрее ответ'] }
    ]
  }
};
